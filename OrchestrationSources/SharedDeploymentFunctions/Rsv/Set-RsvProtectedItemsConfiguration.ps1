...
.EXAMPLE
Set-RsvProtectedItemsConfiguration -RecoveryServicesVaultName 'ProfilesBackupVault' -RecoveryServicesVaultResourceGroup 'resourcegroupname' -fileSharePolicyMaps $fileSharePolicyMaps
    ...
    process {
        ...
        foreach ($fileSharePolicyMap in $fileSharePolicyMaps) {
            ...
            foreach ($item in $fileSharePolicyMap.items) {
                ...
                $backupContainer = Get-AzRecoveryServicesBackupContainer @containerInputObject -ErrorAction 'SilentlyContinue'
                ...
                if (-not $item.FileShareNames) {
                    ...
                    if ($FileShareNames -is [string]) {
                        $relevantFileShareNames += $item.FileShareNames.Split(',')
                    }
                    else {
                        $relevantFileShareNames += $item.FileShareNames
                    }  
                }
