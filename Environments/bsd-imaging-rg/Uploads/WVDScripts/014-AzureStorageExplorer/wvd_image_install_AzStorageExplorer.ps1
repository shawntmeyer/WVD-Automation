...
$SoftwareName = 'AZStorageExplorer'
...
If ($exefiles) {
    ...
    Write-Log -message "Installing '$SoftwareName' via cmdline: '$Exe $Arguments'"
    $Installer = Start-Process -FilePath "$Exe" -ArgumentList $Arguments -Wait -PassThru
...
Else {
    Write-Log -message "No Installer file found to install." -severity 2
}
