...
$Script:Section = "Install Powershell Modules"
Write-Verbose "Installing 'Nuget' Package Provider."
Install-PackageProvider -Name Nuget -Force
Write-Verbose "Installing Powershell Modules."
$Modules = "Az","Az.ImageBuilder"
ForEach ($module in $modules) {
    Try {
        If (-not(Get-InstalledModule | Where-Object {$_.Name -eq '$Module'})) {
            Install-Module -Name $Module -AllowClobber -Force
            Write-Verbose "Completed installing Powershell Module: '$Module'."
        }
    }
    Catch {
        Write-Warning "Error Occured attempting to install Powershell Module: '$Module'."
    }
}
