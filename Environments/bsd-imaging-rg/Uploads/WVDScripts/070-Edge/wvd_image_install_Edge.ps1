...
Function Invoke-LGPO {
    ...
    ForEach ($RegistryFile in $inputFiles) {
        ...
        Write-Log -Message "Applying settings from '$txtFilePath' to Local Group Policy via LGPO.exe." -Source ${CmdletName}
        $null = Start-Process -FilePath "$env:SystemRoot\System32\lgpo.exe" -ArgumentList "/t `"$TxtFilePath`"" -Wait -PassThru
    }
}
...
$null = Expand-Archive $templateszip -DestinationPath $PSScriptRoot -Force
$msifiles = Get-ChildItem -Path "$PSScriptRoot" -File -Filter '*.msi'
If ($msifiles) {
    $EdgeMsi = $msifiles[0].FullName
}
...
If ($($Installer.ExitCode) -eq 0 ) {
    Write-Log -message "'Microsoft Edge' installed successfully."
}
Else {
