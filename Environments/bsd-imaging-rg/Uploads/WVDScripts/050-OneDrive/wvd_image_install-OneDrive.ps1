...
$OneDriveSetup = "$PSScriptRoot\OneDriveSetup.exe"
...
If (Test-Path -Path $OneDriveUninstaller) {
    ...
    Write-Log -Message "OneDrive per-user uninstall completed."
}
Set-RegistryValue -Key "HKLM:\Software\Microsoft\OneDrive" -Name "AllUsersInstall" -Value 1 -Type DWord

Write-Log -message "Starting per machine installation of OneDrive."

$Arguments = "/allusers"
Write-Log -Message "Installing '$SofwareName' via cmdline: 'OneDriveSetup.exe $Arguments'"

$Installer = Start-Process -FilePath $OneDriveSetup -ArgumentList $Arguments -Wait -PassThru


If ($($Installer.ExitCode) -eq 0) {
    Write-Log -message "OneDrive installed successfully."
}
Else {
    Write-Log -message "OneDriveSetup.exe exited with code [$($Installer.ExitCode)]" -severity 2
}
...
Write-Log -message "Configuring OneDrive to start in the background for each user."
...
Write-Log -message "Configuring OneDrive to start in the background when apps accessed through Remote App."
...
Write-Log -Message "Configuring the OneDrive Update Ring to Production"
...
Write-Log -Message "Enabling OneDrive Files on Demand"
