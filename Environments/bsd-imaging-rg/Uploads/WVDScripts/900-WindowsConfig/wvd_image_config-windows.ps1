...
[string]$Script:LogName = "Configure-$SoftwareName.log"
...
## MAIN
Write-Log -message "Starting Windows Configuration"
...
#region Default App Associations
$appassociations = "$PSScriptRoot\AppAssociations\AppAssociations.xml"
If (Test-Path -Path $appassociations){
    Write-Log -Message "Applying default application associations. Ref: 'https://docs.microsoft.com/en-us/windows-hardware/manufacture/desktop/export-or-import-default-application-associations'."
    Start-Process -FilePath "dism.exe" -ArgumentList "/online /Import-DefaultAppAssociations:$appassociations" -ErrorAction SilentlyContinue -Wait
}
#endregion Default App Associations

#region Workplace Join
Write-Log -message "Disabling Workplace Join to prevent issue with Office Activation."
...
#endregion WorkplaceJoin

#region Setup Virtual Desktop Image from Ref
$ref = "https://docs.microsoft.com/en-us/azure/virtual-desktop/set-up-customize-master-image"

Write-Log -message "Configuring Windows in accordance with '$ref'."
...
Invoke-LGPO -SearchTerm "Windows"
Write-Log -message "Completed Windows Configuration."