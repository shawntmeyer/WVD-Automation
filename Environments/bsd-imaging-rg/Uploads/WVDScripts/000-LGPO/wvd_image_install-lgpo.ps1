...
If (Test-Path "$Script:LogDir\$Script:LogName.log") {
    Remove-Item "$Script:LogDir\$Script:LogName.log" -Force
}
#endregion
...
Write-Log -message "Copying '$SoftwareName' utility to system path."
$DestFile = "$env:SystemRoot\System32\lgpo.exe"
$azipfiles = Get-ChildItem -Path $PSScriptRoot -filter '*.zip' -recurse
If ($azipfiles.count -gt 0) {
    $lgpozip = $azipfiles[0].FullName
    Write-Log -message "Expanding '$lgpozip' to '$PSScriptRoot'."
    expand-archive -path "$lgpozip" -DestinationPath "$PSScriptRoot" -force
    $algpoexe = Get-ChildItem -Path $PSScriptRoot -filter 'lgpo.exe' -recurse
    If ($algpoexe.count -gt 0) {
        $lgpoexe=$algpoexe[0].FullName
        Write-Log -message "Copying '$lgpoexe' to '$env:SystemRoot\system32'."
        Copy-Item -Path $lgpoexe -Destination "$env:SystemRoot\System32" -force
        If (Test-Path $DestFile) {
            Write-Log "'$SoftwareName' installed successfully. Exiting Script."
        }
    }
    Else {
        Write-Log -Message "'lgpo.exe' not found in downloaded zip." -severity 3
    }
}
Else {
    Write-Log -Message "No '$softwarename' zip file contained in archive." -severity 3
}    
