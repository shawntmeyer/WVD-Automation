...
if ($env:SYSTEMDRIVE -eq "X:") {
  $script:Offline = $true
  Write-Log -message "Script running in WinPE. Now searching for Offline Windows Drive."
  ...
  Write-Log -message "Eligible offline drive found: $script:OfflinePath"
  ...
  Write-Log -message "Offline Image Build = $Build"
}
else {
  Write-Log -message "Running in the full OS."
  $script:Offline = $false
  [int]$Build = [System.Environment]::OSVersion.Version.Build
  Write-Log -message "Online OS build = $Build"
}
