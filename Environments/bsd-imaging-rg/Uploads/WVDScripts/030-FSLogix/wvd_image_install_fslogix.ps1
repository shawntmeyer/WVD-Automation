...
$azipfiles = Get-ChildItem -Path $PSScriptRoot -filter '*.zip'
If (!$azipfiles) {
    Write-Log -Message "No '$SoftwareName' zip files found in '$PSScriptRoot'." -severity 3
    Exit
}
$FSLogixZip = $aZipFiles[0].FullName
Write-Log -message "Extracting FSLogix Installers from zip."
Expand-Archive $FSLogixZip -DestinationPath $PSScriptRoot -Force
Start-Sleep -Seconds 5
Write-Log -message "Now copying the latest Group Policy ADMX and ADML files to the Policy Definition Folders."
$admx = Get-ChildItem "$PSScriptRoot" -Filter "*.admx" -Recurse
$adml = Get-ChildItem "$PSScriptRoot" -filter "*.adml" -Recurse
...
#region Agent Install
$AgentInstaller = "$PSScriptRoot\x64\release\fslogixappssetup.exe"
If (Test-Path $AgentInstaller) {
    Write-Log -Message "Installation File: '$Agentinstaller' successfully extracted."
}

$Arguments = "/quiet"
Write-Log -Message "Now starting FSLogix Agent installation with command line: '$Agentinstaller $Arguments'."

$Install = Start-Process -FilePath $AgentInstaller -ArgumentList "$Arguments" -Wait -PassThru

Write-Log -message "The fslogixappssetup.exe exit code is [$($Install.ExitCode)]."
#endregion Agent Install

#region Apps Rule Editor Install
$AppsRuleEditorInstaller = "$PSScriptRoot\x64\release\FSLogixAppsRuleEditorSetup.exe"
If (Test-Path $AppsRuleEditorInstaller) {
    Write-Log -Message "Installation File: '$AppsRuleEditorinstaller' successfully extracted."
}

$Arguments = "/quiet"
Write-Log -Message "Now starting FSLogix Apps Rule Editor installation with command line: '$AppsRuleEditorInstaller $Arguments'."

$Install = Start-Process -FilePath $AppsRuleEditorInstaller -ArgumentList "$Arguments" -Wait -PassThru

Write-Log -message "The FSLogixAppsRuleEditorSetup.exe exit code is [$($Install.ExitCode)]."
#endregion


if ( $FSLogixVHDPath -and $FSLogixVHDPath -ne '' ) {
    Write-Log -Message "Now performing FSLogix Configuration."
